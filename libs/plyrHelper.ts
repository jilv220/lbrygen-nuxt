...
import { Observable, fromEvent, Subscription } from "../node_modules/rxjs";
...
function _removeDblClickListeners(player: Plyr) {
  ...
  (player as any).eventListeners!.forEach(function (eventListener: any) {
...
function _bindNewDblClickListeners(
  player: Plyr,
  ...
  if (plyrWrapper && plyrWidth && plyrHeight) {
    ...
    clickSubscription = clickListener.subscribe((e) => {
      ...
      if (isXInRange && isYInRange && isPlyrTooltipShown) {
...
function queryPlyrWidth(
  selector: keyof HTMLElementTagNameMap | keyof SVGElementTagNameMap | string
) {
  const plyrWidth = document
    .querySelector(selector)
    ?.getBoundingClientRect().width;
  return plyrWidth;
}

function queryPlyrHeight(
  selector: keyof HTMLElementTagNameMap | keyof SVGElementTagNameMap | string
) {
  const plyrHeight = document
    .querySelector(selector)
...
function plyrEnableDblClickSeek(
  player: Plyr,
  selector: keyof HTMLElementTagNameMap | keyof SVGElementTagNameMap | string
) {
